    ...
    private void verifyExpression(Expression expression) {
        if (expression instanceof FunctionExpression functionExpression) {
            ...
            if (!def.isVarArgs() && functionExpression.getArguments().length != def.parameterTypes().length) {
                throw new LanguageException("Function " + def.name() + " expects " + def.parameterTypes().length
                        + " arguments, but got " + functionExpression.getArguments().length);
            }
            ...
            for (int i = 0; i < functionExpression.getArguments().length; i++) {
                ...
                if (!expected.isAssignableFrom(actual)) {
                    throw new LanguageException("Argument " + (i + 1) + " of function " + def.name() + " expects type "
                            + expected.getSimpleName() + ", but got " + actual.getSimpleName());
                }
            }
