    ...
    public void onChat(ChatMessageReceivedEvent event) {
        ...
        if (messageMatcher.group(1).equals("sold me")) {
            ...
            for (Component sibling : event.getOriginalMessage().getSiblings()) {
                ...
                Matcher itemMatcher =
                        ITEM_PATTERN.matcher(ComponentUtils.getCoded(sibling).replace("-", "").replace("'", "")); // Second group contains the items. A bit of a hacky solution to remove the pesky "-" and "'" characters that mess up regex matcher
