    ...
    private static void addRealNameToMessage(Component message, Component parentMessage) {
        ...
        if (messageHasNickHover(message)) {
            ...
            if (hover.getValue(hover.getAction()) instanceof Component hoverText) {
                ...
                // Component fullMessage = Component.literal("§r§" + finalColourCode + realName).withStyle(style ->
                // style.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal( "§r§f" + realName
                // + "§r§7" + "'s nickname is " + "§r§f" + ChatFormatting.stripFormatting(message.getString())))));
