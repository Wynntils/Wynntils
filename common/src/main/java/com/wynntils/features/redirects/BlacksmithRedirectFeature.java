...
public class BlacksmithRedirectFeature extends Feature {
    ...
    public void onChat(ChatMessageReceivedEvent event) {
        Matcher messageMatcher =
                event.getOriginalCodedMessage().getMatcher(BLACKSMITH_MESSAGE_PATTERN, PartStyle.StyleType.FULL);
        ...
        if (messageMatcher.group(1).equals("sold me")) {
            ...
            for (Component sibling : event.getOriginalMessage().getSiblings()) {
                ...
                Matcher itemMatcher = StyledText.fromComponent(sibling)
                        .getMatcher(ITEM_PATTERN, PartStyle.StyleType.FULL); // Second group contains the items.
