    ...
    private void renderTexturedCount(GuiGraphics guiGraphics, int x, int y, int emeralds) {
        ...
        for (int i = emeraldAmounts.length - 1; i >= 0; i--) {
            ...
            if (EmeraldUnits.values()[i].getSymbol().equals("stx")) { // Make stx not look like normal LE
                RenderUtils.renderItem(guiGraphics, EmeraldUnits.values()[i].getItemStack(), renderX + 3, renderY + 4);
                RenderUtils.renderItem(guiGraphics, EmeraldUnits.values()[i].getItemStack(), renderX + 6, renderY + 6);
                RenderUtils.renderItem(guiGraphics, EmeraldUnits.values()[i].getItemStack(), renderX + 9, renderY + 8);
            } else {
