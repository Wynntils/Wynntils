...
public class CustomBookshelfPagesFeature extends Feature {
    // Change to ranged integer when implemented
    @RegisterConfig
    public final Config<Integer> buttonOnePage = new Config<>(1);

    @RegisterConfig
    public final Config<Integer> buttonTwoPage = new Config<>(3);

    @RegisterConfig
    public final Config<Integer> buttonThreePage = new Config<>(4);

    @RegisterConfig
    public final Config<Integer> buttonFourPage = new Config<>(6);

    @RegisterConfig
    public final Config<Integer> buttonFivePage = new Config<>(8);

    @RegisterConfig
    public final Config<Integer> buttonSixPage = new Config<>(10);

    private static final int MAX_BOOKSHELF_PAGES = 10;
    ...
    private int lastPage = MAX_BOOKSHELF_PAGES;
    ...
    public void onScreenInit(ScreenInitEvent e) {
        ...
        if (!Models.Container.isBookshelfScreen(screen)) return;
    ...
    public void onContainerSetEvent(ContainerSetContentEvent.Post e) {
        ...
        if (Models.Container.isItemIndicatingLastBankPage(e.getItems().get(Models.Container.LAST_BANK_PAGE_SLOT))) {
            System.out.println("On final page, updating to " + currentPage);
            Models.Container.updateFinalBookshelfPage(currentPage);
    ...
    protected void onConfigUpdate(ConfigHolder configHolder) {
        switch (configHolder.getFieldName()) {
            case "buttonOnePage",
                ...
                } else if ((int) configHolder.getValue() > MAX_BOOKSHELF_PAGES) {
                    configHolder.setValue(MAX_BOOKSHELF_PAGES);
                }
            }
        }
    }
