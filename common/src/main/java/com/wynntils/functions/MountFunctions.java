...
public class MountFunctions {
    private static Optional<CappedValue> getRequestedCappedStat(String statArg) {
        ...
        Optional<MountStat> stat = getRequestedStat(statArg);
        if (stat.isEmpty() || !stat.get().isCapped()) return Optional.empty();

        return Optional.of(getCappedStatValue(mount.get(), stat.get()));
    }

    private static Optional<Integer> getRequestedStatCurrent(String statArg) {
        Optional<MountItem> mount = getMount();
        if (mount.isEmpty()) return Optional.empty();

        return getRequestedStat(statArg).map(stat -> getStatCurrentValue(mount.get(), stat));
    }

    private static Optional<Integer> getRequestedStatMax(String statArg) {
        Optional<MountItem> mount = getMount();
        if (mount.isEmpty()) return Optional.empty();
    ...
    private static Optional<MountStat> getRequestedStat(String statArg) {
    ...
    public static int mountStatFunction(String stat) {
        return getRequestedStatCurrent(stat).orElse(-1);
    }
