...
public final class PartyModel extends Model {
    ...
    private static final Pattern PARTY_OTHER_PROMOTED = Pattern.compile("§eSuccessfully promoted (.+) to party leader!");
    ...
    private static final Pattern PARTY_SELF_DISBAND = Pattern.compile("§eYour party has been disbanded since you were the only member remaining\\.");
    ...
    private boolean tryParsePartyMessages(String coded) {
        ...
        if (PARTY_DISBAND.matcher(coded).matches()
                || PARTY_SELF_LEAVE_MESSAGE_PATTERN.matcher(coded).matches() || PARTY_SELF_ALREADY_LEFT_MESSAGE_PATTERN.matcher(coded).matches() || PARTY_SELF_DISBAND.matcher(coded).matches()) {
