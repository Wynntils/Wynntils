    ...
    private final class TrackedBarHandler implements ClientboundBossEventPacket.Handler {
        ...
        public void add(
            ...
            Optional<Pair<TrackedBar, Matcher>> trackedBarOpt = knownBars.stream()
                    .map(bar -> new Pair<>(
                            bar, StyledText.fromComponent(name).getMatcher(bar.pattern, PartStyle.StyleType.FULL)))
        ...
        public void updateName(UUID id, Component name) {
            handleBarUpdate(id, trackedBar -> {
                Matcher matcher =
                        StyledText.fromComponent(name).getMatcher(trackedBar.pattern, PartStyle.StyleType.FULL);
                if (!matcher.matches()) {
