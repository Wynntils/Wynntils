...
public class CharmTooltipComponent extends IdentifiableTooltipComponent<CharmInfo, CharmInstance> {
    ...
    public TooltipParts buildTooltipParts(
            ItemStack itemStack,
            IdentifiableItemProperty<CharmInfo, CharmInstance> itemProperty,
    ...
    public List<Component> finalizeTooltipLines(
            List<Component> tooltip, int targetWidth, CharmInfo charmInfo, CharmInstance charmInstance) {
        List<Component> finalized = new ArrayList<>(tooltip);
        GearTooltipAlignmentComponent.realignMarkedTooltipLines(finalized);
        return finalized;
    }

    private void updateParsedTitleLine(List<Component> header, CharmInfo charmInfo, CharmInstance charmInstance) {
        if (header.isEmpty()) {
            return;
        }
        ...
        if (emblemTitleIndex > 0
                && header.getFirst().getString().trim().equals(charmInfo.name())
        ...
        MutableComponent styledName = buildRewardItemNameComponent(
                ...
                charmInstance != null && charmInstance.hasOverallValue(),
                charmInstance != null ? charmInstance.getOverallPercentage() : 0f);
        if (!TooltipUtils.replaceTrailingTitleComponent(updatedTitleLine, charmInfo.name(), styledName)) {
