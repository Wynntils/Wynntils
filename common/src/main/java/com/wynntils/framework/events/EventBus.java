...
public class EventBus {
    ...
    public void register(Feature obj) {
        Arrays.stream(obj.getClass().getDeclaredMethods()).forEach(m -> {
                if (!m.isAnnotationPresent(Subscriber.class)) return;
                if (!Modifier.isStatic(m.getModifiers()) || !Modifier.isPublic(m.getModifiers())) return;
                ...
                ListenerHandler handler = subscribers.computeIfAbsent(parameterType, c -> new ListenerHandler());
