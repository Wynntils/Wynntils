...
public class WynnBossBarUtils {
    ...
    public static final AwakenedBar NO_AWAKENED_PROGRESS = new AwakenedBar(0, -1);
    ...
    private static final Pattern AWAKENED_PROGRESS_PATTERN = Pattern.compile("§fAwakening §7\\[§f(\\d+)/200§7]");
    ...
    public static AwakenedBar getAwakenedBar() {
        ...
        try {
            int awakenedProgress = Integer.parseInt(pair.b.group(1));
            float progress = pair.a.getProgress();

            return new AwakenedBar(awakenedProgress, progress);
        } catch (NumberFormatException e) {
    ...
    public record AwakenedBar(int current, float progress) {}
