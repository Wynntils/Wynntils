...
public class DiscoveryManager extends CoreManager {
    ...
    public static void openSecretDiscoveryWiki(DiscoveryInfo discoveryInfo) {
        String wikiUrl = Reference.URLs.buildUrl(
                Reference.URLs.WIKI_TITLE_LOOKUP, Reference.URLs.encodeForWikiTitle(discoveryInfo.getName()));
    ...
    private static void locateSecretDiscovery(String name, DiscoveryOpenAction action) {
        String url =
                Reference.URLs.buildUrl(Reference.URLs.WIKI_DISCOVERY_QUERY, Reference.URLs.encodeForWikiTitle(name));
