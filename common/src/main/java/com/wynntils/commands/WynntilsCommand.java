...
public class WynntilsCommand extends WynntilsCommandBase {
    ...
    private Command<CommandSourceStack> getDonateLink() {
        return context -> {
            ...
            MutableComponent url = new TextComponent("https://www.patreon.com/Wynntils").withStyle(Style.EMPTY
                    .withColor(ChatFormatting.LIGHT_PURPLE)
                    .withUnderlined(true)
                    .withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://www.patreon.com/Wynntils"))
                    .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponent("Click here to open in your browser."))));
    ...
    private Command<CommandSourceStack> getHelp() {
        return context -> {
            MutableComponent text = new TextComponent("").withStyle(Style.EMPTY.withColor(ChatFormatting.GOLD));
...
//            text.append("\n");
    ...
    private Command<CommandSourceStack> getDiscordInvite() {
        return context -> {
            MutableComponent msg = new TextComponent("You're welcome to join our Discord server at:\n").withStyle(ChatFormatting.GOLD);
            String discordInvite = WebManager.getApiUrls() == null ? null : WebManager.getApiUrls().get("DiscordInvite");
            MutableComponent link = new TextComponent(discordInvite == null ? "<Wynntils servers are down>" : discordInvite).withStyle(Style.EMPTY.withColor(ChatFormatting.DARK_AQUA));
            if (discordInvite != null) {
                link.setStyle(link.getStyle()
                        .withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, discordInvite))
                        .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponent("Click here to join our Discord server."))));
            }
    ...
    private static void addCommandDescription(MutableComponent text, String prefix, String name, String description) {
        MutableComponent prefixText = new TextComponent(prefix).withStyle(Style.EMPTY.withColor(ChatFormatting.DARK_GRAY));
        text.append(prefixText);

        MutableComponent nameText = new TextComponent(name).withStyle(Style.EMPTY.withColor(ChatFormatting.RED));
        text.append(nameText);

        text.append(" ");

        MutableComponent descriptionText = new TextComponent(description).withStyle(Style.EMPTY.withColor(ChatFormatting.GRAY));
