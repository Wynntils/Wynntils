...
public class IconSelectionScreen extends WynntilsScreen {
    ...
    private final class IconGridScrollWidget extends ScrollBarWidget {
        private final List<IconGridRowWidget> rows;

        private IconGridScrollWidget(int x, int y, int width, int height) {
            ...
            this.rows = chunkIntoRows(icons);
            updateRowPositions();
        }

        private List<IconGridRowWidget> chunkIntoRows(List<MapIcon> icons) {
            List<IconGridRowWidget> iconGridRowWidgets = new ArrayList<>();
            ...
            for (int i = 0; i < icons.size(); i += GRID_COLUMNS) {
                ...
                iconGridRowWidgets.add(new IconGridRowWidget(rowIcons));
            }
            return iconGridRowWidgets;
        }
        ...
        protected void renderWidgets(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
            rows.forEach(row -> row.render(guiGraphics, mouseX, mouseY, partialTick));
        }
        ...
        public boolean mouseClicked(MouseButtonEvent event, boolean isDoubleClick) {
            ...
            int rowIndex = (int) ((event.y() - viewportTop + scrollOffsetY) / CELL_SIZE);

            if (rowIndex < 0 || rowIndex >= rows.size()) {
                return false;
            }

            IconGridRowWidget row = rows.get(rowIndex);
            return row.mouseClicked(event, isDoubleClick);
        }
    ...
    private final class IconGridRowWidget extends AbstractOptionWidget<List<MapIcon>> {
        private final List<MapIcon> icons;

        private IconGridRowWidget(List<MapIcon> icons) {
            super(Component.literal("icon row"), Component.empty(), CELL_SIZE, null, null);
            this.icons = icons;
        }

        @Override
        protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
            String selectedIconId = IconSelectionScreen.this.tempSelectedIconId;
            ...
            for (int i = 0; i < icons.size(); i++) {
                ...
                if (icon == null) {
                    ...
                    RenderUtils.drawTexturedRect(
                            ...
                            icon.getIdentifier(),
                            CommonColors.WHITE,
                            cellX + (CELL_SIZE - iconWidth) / 2f,
                            cellY + (CELL_SIZE - iconHeight) / 2f,
                            iconWidth,
                            iconHeight,
                            0f,
                            0f,
