...
public final class WynntilsBookSettingsScreen extends WynntilsScreen implements TextboxScreen {
    ...
    private WynntilsBookSettingsScreen() {
        ...
        searchWidget = new SearchWidget(
                95, Texture.CONFIG_BACKGROUND.height() - 32, 100, 20, s -> reloadConfigurableButtons(), this);
    ...
    private void renderConfigTitle(PoseStack poseStack) {
        ...
        FontRenderer.getInstance()
                .renderText(
                        ...
                        Texture.CONFIG_BACKGROUND.width() / 2f / 0.8f + 10,
                        12,
                        CommonColors.BLACK,
    ...
    private static void renderScrollArea(PoseStack poseStack) {
        ...
        RenderUtils.drawTexturedRect(
                poseStack,
                Texture.CONFIG_SCROLL_AREA,
                (Texture.CONFIG_BACKGROUND.width() / 2f - Texture.CONFIG_SCROLL_AREA.width()) / 2f,
                10);
        RenderSystem.disableBlend();
    }

    private static void renderBg(PoseStack poseStack) {
        RenderUtils.drawTexturedRect(poseStack, Texture.CONFIG_BACKGROUND, 0, 0);
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        if (adjustedMouseX <= Texture.CONFIG_BACKGROUND.width() / 2f) {
    ...
    private void reloadConfigurableButtons() {
        ...
        configurableListScrollButton = new ScrollButton(
                23,
                17,
                Texture.CONFIG_BACKGROUND.height() - 50,
                Texture.CONFIG_SCROLL_BUTTON.width(),
                Texture.CONFIG_SCROLL_BUTTON.height() / 2,
                roundedUpPageNeed,
                1,
    ...
    private void reloadConfigButtons() {
        ...
        for (int i = 0; i < configsOptions.size(); i++) {
            ...
            configs.add(new ConfigTile(
                    Texture.CONFIG_BACKGROUND.width() / 2 + 10, 21 + renderIndex * 46, 160, 45, this, config));
        }

        int roundedUpPageNeed = configs.size() / CONFIGS_PER_PAGE + (configs.size() % CONFIGS_PER_PAGE == 0 ? 0 : 1);
        configListScrollButton = new ScrollButton(
                Texture.CONFIG_BACKGROUND.width() - 23,
                17,
                Texture.CONFIG_BACKGROUND.height() - 25,
                Texture.CONFIG_SCROLL_BUTTON.width(),
                Texture.CONFIG_SCROLL_BUTTON.height() / 2,
                configs.size() <= CONFIGS_PER_PAGE ? 0 : (roundedUpPageNeed - 1) * CONFIGS_PER_PAGE,
                CONFIGS_PER_PAGE,
    ...
    private float getTranslationY() {
        return (this.height - Texture.CONFIG_BACKGROUND.height()) / 2f;
    }

    private float getTranslationX() {
        return (this.width - Texture.CONFIG_BACKGROUND.width()) / 2f;
    }
