...
public class QualityTierSortButton extends GuideSortButton {
    public QualityTierSortButton(WynntilsGuideScreen guideScreen, ItemSearchQuery searchQuery) {
        super(guideScreen);
    ...
    public void updateFromQuery(ItemSearchQuery searchQuery) {
        Optional<SortInfo> sortInfoOptional = searchQuery.sorts().stream()
                .filter(sortInfo -> sortInfo.provider() instanceof QualityTierStatProvider)
                .findFirst();
    ...
    protected SortInfo getSortInfo() {
        ...
        return new SortInfo(sortDirection, new QualityTierStatProvider());
    }

    @Override
    protected String getSortName() {
        return I18n.get("service.wynntils.itemFilter.stat.qualityTier.name");
    }
}
