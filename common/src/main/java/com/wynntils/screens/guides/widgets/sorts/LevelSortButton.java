...
public class LevelSortButton extends GuideSortButton {
    public LevelSortButton(WynntilsGuideScreen guideScreen, ItemSearchQuery searchQuery) {
        super(guideScreen);
    ...
    public void updateFromQuery(ItemSearchQuery searchQuery) {
        Optional<SortInfo> sortInfoOptional = searchQuery.sorts().stream()
                .filter(sortInfo -> sortInfo.provider() instanceof LevelStatProvider)
                .findFirst();
    ...
    protected SortInfo getSortInfo() {
        ...
        return new SortInfo(sortDirection, new LevelStatProvider());
    }

    @Override
    protected String getSortName() {
        return I18n.get("service.wynntils.itemFilter.stat.level.name");
    }
}
