...
public final class ItemSharingScreen extends WynntilsScreen {
    ...
    private void shareItem(String target) {
        switch (target) {
            case "guild" -> Handlers.Command.sendCommandImmediate("g " + encodedItem.toUtf16String());
            case "party" -> Handlers.Command.sendCommandImmediate("p " + encodedItem.toUtf16String());
            case "save" -> {
