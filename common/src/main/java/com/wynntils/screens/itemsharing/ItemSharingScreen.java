...
public final class ItemSharingScreen extends WynntilsScreen {
    ...
    private void shareItem(String target) {
        switch (target) {
            case "guild" ->
                Handlers.Command.queueCommand("g " + Models.ItemEncoding.makeItemString(wynnItem, encodedItem));
            case "party" ->
                Handlers.Command.queueCommand("p " + Models.ItemEncoding.makeItemString(wynnItem, encodedItem));
            case "save" -> {
